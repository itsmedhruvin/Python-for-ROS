...
# from sensor_msgs.msg import Imu     # imu message # fastbot has no imu
# standard imports
import math
import subprocess
...
class RobotInterface(Node):
    ...
    def __init__(self):
        ...
        self.is_sim = (True if ("/gazebo" in nodes) else False)
        self.get_logger().info("Environment: " + ("Simulation" if self.is_sim else "Real-Robot"))
        ...
        if (self.is_sim):
            ...
            self.lin_vel_min = 0.000
            self.lin_vel_max = 0.150
            self.ang_vel_min = 0.000
            self.ang_vel_max = 0.450
        else:
            # real-robot - fastbot
            self.lin_vel_min = 0.050
            self.lin_vel_max = 0.250
            self.ang_vel_min = 0.500
            self.ang_vel_max = 1.500
    ...
    def publish_twist_cmd(self):
        ...
        direction_linear = (-1.0 if (self.twist_cmd.linear.x < 0.0) else 1.0)
        velocity_linear = abs(self.twist_cmd.linear.x)
        if (velocity_linear < self.lin_vel_min):
            velocity_linear = 0.000
        else:
            pass
        if (velocity_linear > self.lin_vel_max):
            velocity_linear = self.lin_vel_max
        else:
            pass
        self.twist_cmd.linear.x = (velocity_linear * direction_linear)
        # angular speed control
        direction_angular = (-1.0 if (self.twist_cmd.angular.z < 0.0) else 1.0)
        velocity_angular = abs(self.twist_cmd.angular.z)
        if (velocity_angular < self.ang_vel_min):
            velocity_angular = 0.000
        else:
            pass
        if (velocity_angular > self.ang_vel_max):
            velocity_angular = self.ang_vel_max
        else:
            pass
        self.twist_cmd.angular.z = (velocity_angular * direction_angular)
        # publish command
